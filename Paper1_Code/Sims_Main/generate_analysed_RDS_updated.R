...
extract_fitnesses<-function(data,rel_log_gen){
  ...
  data2use=lapply(gendata,function(q){
    ...
    env_microbe_fitness_changes_var<-var((q$env_fits))
    host_microbe_fitness_changes<-mean((q$MicrobeFitness))#/x[[1]]$GenData[[500]]$MicrobeFitness))
    host_microbe_fitness_changes_var<-var((q$MicrobeFitness))
    host_fitness_changes<-mean((q$HostFitness))#/x[[1]]$GenData[[500]]$HostFitness))
    host_fitness_changes_var<-var((q$HostFitness))#/x[[1]]$GenData[[500]]$HostFitness))
    ...
    host_microbe_coeff=lm(q$MicrobeFitness~q$HostFitness)
      mean_microbe_traitval_mean=(q$mean_microbe_traitval)
    ...
    data.frame(EnvMicrobeFitness=env_microbe_fitness_changes,
               ...
               adj_rsq=adj_rsq,
               host_bdiv=host_bdiv,
              envmicrobe_fitvar=env_microbe_fitness_changes_var,
               hostmicrobe_fitvar=host_microbe_fitness_changes_var,
               host_fitvar=host_fitness_changes_var,
               mean_microbe_traitval=mean_microbe_traitval_mean
              )
  }
  )
...
##
#setwd("/nesi/nobackup/uoa04039/holobiont_first_paper_sim_reps/newsims_bdiv/")
#file="largererun_results_multigen_replicate_1.RDS"
#env_file<-"largererun_sec_varyring_envs_multigen_replicate_1.RDS"
###
...
#  gen_rich_varenv<-gen_rich_varenv %>% mutate(NGens=factor(NGens,levels=c("1gen","2gen","3gen","4gen","5gen","6gen","7gen","8gen","9gen","10gen","20gen","50gen","100gen","200gen")))
      gen_rich_varenv<-gen_rich_varenv %>% mutate(NGens=factor(NGens,levels=c("1gen","2gen","3gen","4gen","5gen","6gen","7gen","8gen","9gen","10gen","20gen","50gen","100gen","200gen")))
