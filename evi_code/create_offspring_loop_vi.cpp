...
List offspring_loopfunc(NumericMatrix host_pop, NumericVector ENV_sampling_probability, NumericVector host_microbe_optima,
                                 ...
                                 int n_micro, StringVector microbe_names){ //, NumericVector fitness_microbes, NumericVector weighted_samplingprob) {
  ...
  for (int i = 0; i < n_hosts; i++) {
    ...
    microbe_probs(_, 4) = microbe_probs(_, 3)  + microbe_probs(_, 2); 
    for (int z = 0; z < N_Species; ++z) {
      microbe_probs(z, 6) = fitness_func_bacgen_cpp(selection_parameter_microbes, host_microbe_optima[i], env_condition, microbe_trait_list[z]);
    }
    ...
    microbe_probs(_, 4) = sampling_probability;
    ...
    fitness_microbes[i] = sum(microbe_probs(_, 6) * microbe_probs(_, 9)) / sum(microbe_probs(_, 9));
    weighted_samplingprob[i] = sum(microbe_probs(_, 4) * microbe_probs(_, 9)) / sum(microbe_probs(_, 9));
  }
...
List offspring_loopfunc_vi(NumericMatrix host_pop, NumericVector ENV_sampling_probability, NumericVector host_microbe_optima,
  ...
  for (int i = 0; i < n_hosts; i++) {
    NumericVector parents = host_pop(_, i);
    NumericMatrix microbe_probs(N_Species, 10);
    rownames(microbe_probs) = microbe_names;
    
    microbe_probs(_, 0) = parents / n_micro;
    ...
    microbe_probs(_, 1) = ENV_sampling_probability;
    microbe_probs(_, 2) = X * microbe_probs(_, 0);
    microbe_probs(_, 3) = (1 - X) * microbe_probs(_, 1);
    microbe_probs(_, 4) = microbe_probs(_, 3) + microbe_probs(_, 2);
    
    for (int z = 0; z < N_Species; ++z) {
      microbe_probs(z, 6) = fitness_func_bacgen_cpp(selection_parameter_microbes, host_microbe_optima[i], env_condition, microbe_trait_list[z]);
    }
    microbe_probs(_, 5) = microbe_probs(_, 6);
    NumericVector sampling_probability = microbe_probs(_, 5) * microbe_probs(_, 4);
    sampling_probability = sampling_probability / sum(sampling_probability);
    microbe_probs(_, 4) = sampling_probability;
    NumericVector sampled_counts = rmultinom_cpp(n_micro, sampling_probability);
    microbe_probs(_, 9) = sampled_counts;
    
    host_pop(_, i) = sampled_counts;
    fitness_microbes[i] = sum(microbe_probs(_, 6) * microbe_probs(_, 9)) / sum(microbe_probs(_, 9));
    weighted_samplingprob[i] = sum(microbe_probs(_, 4) * microbe_probs(_, 9)) / sum(microbe_probs(_, 9));
  }
